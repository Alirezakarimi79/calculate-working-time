...
jQuery.fn.hip = function (param) {
    ...
    if (objects.length > 0) {
        ...
        jQuery.each(objects, function (key, curObj) {
            ...
            var pagination = '<div class="hip-pagination row mx-0">' +
                '<a href="#prev" class="d-none">&laquo;</a>';
            for (var i = 1; i <= page_length; i++) {
                if (i === 1) {
                    pagination += '<a class="active" href="#' + i + '">' + curObj.children[i - 1].attributes.userLastName.nodeValue + '</a>';
                }
                else {
                    pagination += '<a href="#' + i + '">' + curObj.children[i - 1].attributes.userLastName.nodeValue + '</a>';
                }

            }
            pagination += '<a href="#next" class="d-none">&raquo;</a>\n' +
                '</div>';
